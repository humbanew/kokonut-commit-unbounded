...
function resolveHookDir(customPath?: string) {
    return customPath
        ? path.resolve(process.cwd(), customPath)
        : path.join(process.cwd(), '.git', 'hooks')
}

function runGitConfig(argsToRun: string[]) {
    const result = spawnSync('git', argsToRun, { encoding: 'utf8' })
    if (result.status !== 0) {
        const message = (
            result.stderr ||
            result.stdout ||
...
export async function promptForMissingApiKey(): Promise<boolean> {
    ...
    const provider =
        (cfg.provider as string) || process.env.KOKONUT_PROVIDER || 'openai'
    const apiKey = (cfg.apiKey as string) || process.env.KOKONUT_API_KEY || ''
    ...
    try {
        ...
        const entered = await rl.question(
            'Enter your API key (or leave empty to cancel): '
        )
...
function writeConfig(key: string, value: unknown, globalFlag: boolean) {
...
if (cmd === 'setup') {
    ...
    async function interactiveSetup() {
        ...
        try {
            ...
            const apiKey = await rl.question(
                'API key (leave empty to cancel): '
            )
            if (!apiKey.trim()) {
                console.log('Setup cancelled')
                process.exit(0)
            }
            const saveGlobalInput = await rl.question(
            ...
            console.log(
                `Setup saved to ${saveGlobal ? '~/.kokonutrc' : './.kokonutrc'}`
            )
        } finally {
            try {
                rl.close()
    ...
    if (providerIndex !== -1 && apiKeyIndex !== -1) {
        ...
        if (url) writeConfig('apiUrl', url, globalFlagArg)
        if (temperatureIndex !== -1)
            writeConfig(
                'temperature',
                parseFloat(args[temperatureIndex + 1]),
                globalFlagArg
            )
        if (topPIndex !== -1)
        ...
        if (frequencyPenaltyIndex !== -1)
            writeConfig(
                'frequencyPenalty',
                parseFloat(args[frequencyPenaltyIndex + 1]),
                globalFlagArg
            )
        if (presencePenaltyIndex !== -1)
            writeConfig(
                'presencePenalty',
                parseFloat(args[presencePenaltyIndex + 1]),
                globalFlagArg
            )
        ...
        console.log(
            `Setup saved to ${globalFlagArg ? '~/.kokonutrc' : './.kokonutrc'}`
        )
        process.exit(0)
    }
    ...
    ;(async () => {
        ...
        try {
            if (provider === 'openai') {
                ...
                type OpenAIModelsResp = { data?: Array<{ id?: string }> }
                const maybe = data as OpenAIModelsResp
            ...
            else console.error('Failed to list models:', String(e))
        }
        process.exit(0)
    })()
}
...
if (cmd === 'hook') {
    ...
    if (!sub) {
        console.error(
            'Usage: kokonut-commit hook set|unset [--global] [--usr <path>]'
        )
        process.exit(1)
    }
    ...
    const customHookDir =
        customHookDirIndex !== -1 ? args[customHookDirIndex + 1] : undefined
    const targetCustomHooksDir = resolveHookDir(customHookDir)
    const targetCustomHook = path.join(
    ...
    if (sub === 'set') {
        ...
        if (hasCustomDirFlag) {
            ...
            try {
                ...
                console.error(
                    'Failed to configure custom hook path:',
                    (e as Error).message
                )
                process.exit(1)
            }
    ...
    if (sub === 'unset') {
        try {
                    ...
                    try {
                        ...
                        if (stats.isSymbolicLink()) {
                            ...
                            if (real === preferredCli) {
                                ...
                                console.log(
                                    'Removed global prepare-commit-msg hook (symlink)'
                                )
                            } else {
                                console.log(
                                    'Global prepare-commit-msg exists and is not the kokonut symlink; not removing'
                                )
                            }
                        } else {
                            fs.unlinkSync(targetGlobalHook)
                            console.log(
                                'Removed global prepare-commit-msg hook'
                            )
                        }
                    } catch (e) {
                        console.error(
                            'Failed to remove global hook:',
                            (e as Error).message
                        )
                        process.exit(1)
                    }
                    ...
                    try {
                        ...
                        if (stats.isSymbolicLink()) {
                            ...
                            if (real === preferredCli) {
                                ...
                                console.log(
                                    'Removed custom prepare-commit-msg hook (symlink)'
                                )
                            } else {
                                console.log(
                                    'Custom prepare-commit-msg exists and is not the kokonut symlink; not removing'
                                )
                            }
                        } else {
                            fs.unlinkSync(targetCustomHook)
                            console.log(
                                'Removed custom prepare-commit-msg hook'
                            )
                        }
                    } catch (e) {
                        console.error(
                            'Failed to remove custom hook:',
                            (e as Error).message
                        )
                        process.exit(1)
                    }
                ...
                try {
                    ...
                    if (stats.isSymbolicLink()) {
                        ...
                        if (real === preferredCli) {
                            ...
                            console.log(
                                'Removed .git/hooks/prepare-commit-msg (symlink)'
                            )
                        } else {
                            console.log(
                                'prepare-commit-msg exists and is not the kokonut symlink; not removing'
                            )
                        }
                    } else {
            ...
            console.error('Failed to remove hook:', (err as Error).message)
            process.exit(1)
        }
    }
