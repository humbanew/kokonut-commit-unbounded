...
function createAIProvider(providerIndex, apiToken, commitPreset = 'default') {
...
${presetInstructions}`;
    ...
    function addEmojiToMessage(message) {
        ...
        const typeMatch = message.match(/^(feat|fix|docs|style|refactor|perf|test|chore|ci|revert)/);
        if (typeMatch) {
            const type = typeMatch[1];
            const emoji = EMOJI_MAP[type] || '';
            if (emoji)
                return message.replace(/^(\w+)/, `${emoji} $1`);
        }
        return message;
    }
    ...
    switch (providerIndex) {
        case 0: // Google Gemini
            return {
                name: 'Google Gemini',
                async generateCommitMessage(diff, originalMessage, includeEmojis, topK, topP, _commitPreset) {
                    const response = await fetchWithTimeout('https://generativelanguage.googleapis.com/v1beta/models/gemini-pro:generateContent?key=' +
                        apiToken, {
                        method: 'POST',
...
function getDefaults() {
    const versionInfo = CONFIG_VERSIONS.find((v) => v.version === CURRENT_VERSION);
    if (!versionInfo) {
...
function validateConfig(config) {
    ...
    if (config.provider !== undefined) {
        if (!Number.isInteger(config.provider) ||
            config.provider < 0 ||
            config.provider > 6) {
    ...
    if (config.fallbackProviders !== undefined) {
        if (!Array.isArray(config.fallbackProviders)) {
            errors.push('Invalid fallbackProviders: must be an array of provider indexes');
        }
        else if (!config.fallbackProviders.every((provider) => Number.isInteger(provider) && provider >= 0 && provider <= 6)) {
            errors.push('Invalid fallbackProviders: all items must be integers between 0 and 6');
        }
    }
    if (config.temperature !== undefined) {
        if (typeof config.temperature !== 'number' ||
            ...
            errors.push(`Invalid temperature: ${config.temperature} (must be 0-2)`);
        }
    }
    ...
    if (config.topK !== undefined) {
        if (!Number.isInteger(config.topK) ||
            config.topK < 0 ||
            config.topK > 100) {
    ...
    if (config.maxTokens !== undefined) {
        if (!Number.isInteger(config.maxTokens) ||
            ...
            errors.push(`Invalid maxTokens: ${config.maxTokens} (must be 1-65536 integer)`);
        }
    }
    if (config.frequencyPenalty !== undefined) {
        if (typeof config.frequencyPenalty !== 'number' ||
            ...
            config.frequencyPenalty > 2) {
            errors.push(`Invalid frequencyPenalty: ${config.frequencyPenalty} (must be -2 to 2)`);
        }
    }
    if (config.presencePenalty !== undefined) {
        if (typeof config.presencePenalty !== 'number' ||
            ...
            config.presencePenalty > 2) {
            errors.push(`Invalid presencePenalty: ${config.presencePenalty} (must be -2 to 2)`);
        }
    }
    if (config.stopSequences !== undefined) {
        ...
        else if (!config.stopSequences.every((seq) => typeof seq === 'string')) {
            errors.push(`Invalid stopSequences: all items must be strings`);
        }
        else if (config.stopSequences.length > 4) {
            errors.push(`Invalid stopSequences: maximum 4 stop sequences allowed`);
        }
    }
    if (config.prefix !== undefined && typeof config.prefix !== 'string') {
        errors.push(`Invalid prefix: ${config.prefix} (must be string)`);
    }
    if (config.includeEmojis !== undefined &&
        typeof config.includeEmojis !== 'boolean') {
        errors.push(`Invalid includeEmojis: ${config.includeEmojis} (must be boolean)`);
    }
    if (config.commitPreset !== undefined &&
        typeof config.commitPreset !== 'string') {
        errors.push(`Invalid commitPreset: ${config.commitPreset} (must be string)`);
    }
...
function readWorkspaceConfig() {
    ...
    try {
        ...
        const fallbackProviders = typeof rawConfig.fallbackProviders === 'string'
        ...
        return {
            ...
            fallbackProviders,
            prefix: typeof rawConfig.prefix === 'string'
                ? rawConfig.prefix
                : undefined,
            includeEmojis: normalizeBoolean(rawConfig.includeEmojis),
            commitPreset: typeof rawConfig.commitPreset === 'string'
        ...
        core.warning(`Unable to read workspace config: ${error instanceof Error ? error.message : String(error)}`);
        return {};
    }
}
function parseProviderIndex(value) {
...
function readBooleanPreference(inputName, fallback) {
    const rawInput = core.getInput(inputName);
    if (rawInput !== '') {
        const val = rawInput.trim();
        if (/^(true|1|yes)$/i.test(val))
            return true;
        if (/^(false|0|no)$/i.test(val))
            return false;
...
function resolveSelectedProviderIndex(providers, fallbackProviderIndex) {
    const selectedIndex = providers.findIndex((p) => p);
    if (selectedIndex !== -1) {
        return selectedIndex;
    }
    if (fallbackProviderIndex !== undefined &&
...
function estimateConfidence(diff, originalMessage) {
    ...
    const confidence = Math.max(0.15, Math.min(0.95, 0.45 + lengthScore + complexityScore + messageScore));
    const confidenceLabel = confidence >= 0.8 ? 'high' : confidence >= 0.6 ? 'medium' : 'low';
    return { confidence, confidenceLabel };
}
function uniqueProviderCandidates(primaryIndex, fallbackProviders) {
    return [primaryIndex, ...fallbackProviders].filter((providerIndex, position, providerIndices) => providerIndices.indexOf(providerIndex) === position &&
        providerIndex >= 0 &&
 ...
 */
function validateSingleProvider(providers, fallbackProviderIndex) {
    ...
    if (selectedCount > 1) {
        core.setFailed('Multiple AI providers selected! Please select only one provider.');
 ...
 */
async function getCommitDiff(octokit, owner, repo, commitSha) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
 ...
 */
async function createCommitSuggestionsComment(octokit, owner, repo, pullNumber, suggestions) {
    ...
    const renderSuggestionTable = (group) => group
        .map((suggestion) => `| ${suggestion.sha.substring(0, 7)} | ${suggestion.providerName} | ${suggestion.confidenceLabel} | ${suggestion.oldMessage} | ${suggestion.newMessage} |`)
        .join('\n');
 ...
 */
async function processCommits(octokit, owner, repo, pullNumber, aiProvider, aiTokenApi, providerIndex, fallbackProviders, autoRename, includeEmojis, commitPrefix, topK, topP, commitPreset) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
        ...
        if (!isConventionalCommit(originalMessage)) {
            ...
            if (autoRename) {
                try {
                    ...
                    const providerCandidates = uniqueProviderCandidates(providerIndex, fallbackProviders);
                    let improvedMessage = '';
                    let providerName = aiProvider.name;
                    for (const candidateIndex of providerCandidates) {
                        const candidateProvider = createAIProvider(candidateIndex, aiTokenApi, commitPreset);
                        try {
                            improvedMessage =
                                await candidateProvider.generateCommitMessage(diff, originalMessage, includeEmojis, topK, topP, commitPreset);
                            providerName = candidateProvider.name;
                            if (candidateIndex !== providerIndex) {
                                core.warning(`Provider fallback used for ${sha.substring(0, 7)}: ${providerName}`);
                            }
                            break;
                        }
                        catch (error) {
                            ...
                            core.warning(`Provider ${candidateProvider.name} failed for ${sha.substring(0, 7)}; trying fallback provider.`);
                        }
                    }
                    ...
                    core.info(`✓ Improved: "${originalMessage}" → "${improvedMessage}"`);
                }
                catch (error) {
                    core.warning(`Failed to rename commit ${sha.substring(0, 7)}: ${error instanceof Error ? error.message : String(error)}`);
                }
            }
        }
    }
...
async function run() {
    ...
    const providerFallback = (index) => runtimeConfig.provider !== undefined
        ? runtimeConfig.provider === index
        : undefined;
    const isGoogleGemini = readBooleanPreference('isGoogleGemini', providerFallback(0));
    const isOpenAIChatGPT = readBooleanPreference('isOpenAIChatGPT', providerFallback(1));
    const isAnthropicClaude = readBooleanPreference('isAnthropicClaude', providerFallback(2));
    const isMistralLeChat = readBooleanPreference('isMistralLeChat', providerFallback(3));
    ...
    const isHuggingFace = readBooleanPreference('isHuggingFace', providerFallback(6));
    ...
    const includeEmojis = readBooleanPreference('includeEmojis', runtimeConfig.includeEmojis);
    const dryRun = readBooleanPreference('dryRun', runtimeConfig.dryRun);
    const commitPrefix = core.getInput('commitPrefix') || runtimeConfig.prefix || '';
    const commitPreset = core.getInput('commitPreset') || runtimeConfig.commitPreset || 'default';
    ...
    try {
        ...
        const selectedProvider = validateSingleProvider(providers, fallbackProviderIndex);
        ...
        const providerIndex = resolveSelectedProviderIndex(providers, fallbackProviderIndex);
        const aiProvider = createAIProvider(providerIndex, aiTokenApi, commitPreset);
        ...
        if (eventData.pull_request?.number) {
            ...
            core.info(`Processing commits for ${owner}/${repo}#${pullNumber}...`);
            if (autoRenameCommits) {
                core.info(`🤖 Auto-rename enabled | Emojis: ${includeEmojis ? '✓' : '✗'} | Mode: ${renameMode}`);
            }
            // Process commits
            const result = await processCommits(octokit, owner, repo, pullNumber, aiProvider, aiTokenApi, providerIndex, fallbackProviders, autoRenameCommits, includeEmojis, commitPrefix, topK, topP, commitPreset);
            if (result.nonCompliant.length > 0) {
                core.warning(`Found ${result.nonCompliant.length} commit(s) not following Conventional Commits:`);
                result.nonCompliant.forEach((commit) => {
                    core.warning(`  - ${commit.sha.substring(0, 7)}: ${commit.message}`);
                });
                if (!autoRenameCommits) {
                    core.setFailed('Some commits do not follow the Conventional Commits standard!');
                }
            }
            else {
            ...
            if (result.renamed.length > 0) {
                core.info(`✓ Successfully improved ${result.renamed.length} commit(s)`);
                core.setOutput('renamedCommits', String(result.renamed.length));
                core.setOutput('improvedMessages', JSON.stringify(result.renamed.map((r) => ({
                    sha: r.sha,
                    old: r.oldMessage,
                    new: r.newMessage,
                ...
                core.setOutput('confidenceSummary', JSON.stringify(result.renamed.map((commit) => ({
                    sha: commit.sha,
                ...
                if (dryRun) {
                    core.setOutput('previewSuggestions', JSON.stringify(result.renamed));
                    core.info('Dry-run enabled: preview outputs generated only.');
                }
                // Create PR comment if in PR mode
                if (renameMode === 'pr' && !dryRun) {
                    const prCommentUrl = await createCommitSuggestionsComment(octokit, owner, repo, pullNumber, result.renamed);
                    if (prCommentUrl) {
                    ...
                    core.info('⚠️ Force mode requires manual git operations. Use output messages to apply changes.');
                }
            }
        }
    }
